...
async def test_filter_based_search():
    ...
    try:
        ...
        test_queries = [
            ...
            # B. 수신자 필터 테스트  
            "dw_191-MBK_all에게 발송된 메시지를 모두 찾아줘",
            ...
            if result == "success":
                ...
                try:
                    ...
                    search_result = await tools.get_document_with_filter(
                        class_name="DocumentChunk",
                        limit=5,
                        filters={}  # 실제 필터는 extract_filter_from_query에서 이미 처리됨
                    )
                    
                    if search_result and len(search_result) > 0:
                        print(f" 검색 결과: {len(search_result)}개 문서 발견")
                        for j, doc in enumerate(search_result[:3], 1):
                            print(f"  {j}. {doc.get('ori_file_name', 'N/A')}")
                    else:
                        print("📄 검색 결과: 문서 없음")
                        
                except Exception as search_error:
                    print(f"❌ 검색 실행 오류: {search_error}")
