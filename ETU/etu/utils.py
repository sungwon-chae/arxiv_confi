...
def build_forbidden_token_ids_pmi(
    ...
    manual_vs_path: Optional[str] = None,
) -> List[int]:
    """
    ...
    Mode: Manual V_S 우선 로드 → PMI-based refinement (fallback)
    """
    import os
    
    # 1. 수동 V_S 파일 우선 확인
    if manual_vs_path and os.path.exists(manual_vs_path):
        print(f"[V_S] 수동 V_S 파일 로드: {manual_vs_path}")
        try:
            with open(manual_vs_path, 'r', encoding='utf-8') as f:
                manual_vs = json.load(f)
            
            # V_S 파일 형식 처리: 리스트 또는 {"token_ids": [...]} 형식
            if isinstance(manual_vs, list) and len(manual_vs) > 0:
                token_list = manual_vs
            elif isinstance(manual_vs, dict) and "token_ids" in manual_vs:
                token_list = manual_vs["token_ids"]
            else:
                token_list = []
                
            if len(token_list) > 0:
                print(f"[V_S] 수동 V_S 로드 성공: {len(token_list)}개 토큰")
                # 수동 V_S 토큰들을 검증하고 필터링
                valid_manual_vs = []
                for token_id in token_list:
                    if isinstance(token_id, int) and 0 <= token_id < len(tokenizer):
                        valid_manual_vs.append(token_id)
                
                if valid_manual_vs:
                    print(f"[V_S] 유효한 수동 V_S: {len(valid_manual_vs)}개 토큰")
                    return _filter_vs_tokens(tokenizer, valid_manual_vs)
                else:
                    print(f"[V_S] 수동 V_S에 유효한 토큰이 없음, PMI fallback 사용")
            else:
                print(f"[V_S] 수동 V_S가 비어있음, PMI fallback 사용")
        except Exception as e:
            print(f"[V_S] 수동 V_S 로드 실패: {e}, PMI fallback 사용")
    else:
        print(f"[V_S] 수동 V_S 파일 없음, PMI fallback 사용")
    
    # 2. Fallback: 기존 PMI-based 방식
    print(f"[V_S] PMI-based V_S 구축 시작...")
    if max_batches_per_split is not None:
...
def build_forbidden_token_ids(forget_data_list, tokenizer, vocab_top_k: Optional[int] = None,
                             rate: float = 0.01, abs_cap: int = 20000, manual_vs_path: Optional[str] = None):
    """
    Build forbidden token set V_S from forget data with frequency filtering.
    
    Mode: Manual V_S 우선 로드 → Frequency-based filtering (fallback)
    - 먼저 수동 V_S 파일을 확인하고 로드
    - 수동 V_S가 없으면 기존 frequency-based 방식 사용
        ...
        try:
            ...
            if len(token_list) > 0:
                ...
                if valid_manual_vs:
                    ...
                    print(f"[V_S] 수동 V_S에 유효한 토큰이 없음, fallback 사용")
            else:
                print(f"[V_S] 수동 V_S가 비어있음, fallback 사용")
        except Exception as e:
            print(f"[V_S] 수동 V_S 로드 실패: {e}, fallback 사용")
    else:
        print(f"[V_S] 수동 V_S 파일 없음, fallback 사용")
    
    # 2. Fallback: 기존 frequency-based 방식
    print(f"[V_S] Frequency-based V_S 구축 시작...")
    cnt = Counter()
    for forget_data in forget_data_list:
