...
def create_enhanced_prompt(user_message, selected_options, selected_tasks=None, task_params=None, use_context_v2=False):
    ...
    # --- 유틸: 샘플 선별 로직 (v2: 개선 버전) ---
    def select_samples_for_style(option_context, target_orientation, k=2):
        """
        Style Transfer용 샘플 선택 로직 (v2: 개선 버전)
        - 목표 점수에 정확히 일치하는 샘플을 우선 선택
        - 부족하면 인접 점수로 보완
        
        Args:
            option_context: SPA 용어 컨텍스트
            target_orientation: 'seller' (매도인 친화, 목표 4점) 또는 'buyer' (매수인 친화, 목표 0점)
            k: 선택할 샘플 개수 (기본값: 2)
        ...
        target_score = 4.0 if target_orientation == "seller" else 0.0
        
        # 점수가 있는 샘플만 필터링
        scored_samples = [s for s in samples if isinstance(s.get("score"), (int, float))]
        
        if not scored_samples:
            return []
        
        # 1단계: 목표 점수에 정확히 일치하는 샘플 우선 선택
        exact_matches = [s for s in scored_samples if float(s.get("score")) == target_score]
        
        # 2단계: 정확히 일치하는 샘플이 부족하면, 점수 차이 기준으로 가까운 샘플 선택
        if len(exact_matches) < k:
            # 정확히 일치하는 샘플 제외하고 나머지 샘플에서 선택
            remaining_samples = [s for s in scored_samples if s not in exact_matches]
            remaining_samples.sort(key=lambda s: abs(float(s.get("score")) - target_score))
            
            # 필요한 만큼 추가 선택
            needed = k - len(exact_matches)
            selected = exact_matches + remaining_samples[:needed]
        else:
            # 정확히 일치하는 샘플이 충분하면 그 중에서만 선택
            selected = exact_matches[:k]
