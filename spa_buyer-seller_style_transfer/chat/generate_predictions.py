...
# -*- coding: utf-8 -*-
"""
실제 LLM을 사용하여 Style Transfer 예측 생성
GPU에 올린 온프레미스 LLM (Qwen)을 사용하여 test_data의 각 항목에 대해 예측 생성
"""
...
def get_llm(temperature: float = 0.7, max_tokens: int = 32768):
    ...
    return ChatOpenAI(
        ...
        timeout=120.0,  # 예측 생성은 시간이 걸릴 수 있으므로 타임아웃 증가
    )
...
def generate_prediction_for_item(
    ...
    # - generate_predictions.py: 직접 LLM 호출 → Pydantic 모델 → model_dump() → 딕셔너리
    try:
        ...
        if output_schema:
            ...
            # generate_predictions.py: 직접 Pydantic 모델을 받으므로 model_dump() 사용
...
def main():
    ...
    parser = argparse.ArgumentParser(description='LLM을 사용한 Style Transfer 예측 생성')
    parser.add_argument('--test_data', type=str, required=True,
                       help='테스트 데이터 JSON 파일 경로')
    parser.add_argument('--output', type=str, default=None,
                       help='예측 결과 저장 경로 (기본값: predictions_<timestamp>.json)')
    ...
    if args.output is None:
        ...
        args.output = f"predictions_utils{utils_ver}_ctx{context_ver}_{timestamp}.json"
